...
export default function AnalyticsPage() {
  ...
  const modelPieData = aggregated.reduce((acc, row) => {
  ...
  const providerBarData = aggregated.reduce((acc, row) => {
  ...
  const tokenModelPieData = tokenStats.reduce((acc, row) => {
    const existing = acc.find(item => item.name === row.model);
    if (existing) {
  ...
  const tokenProviderBarData = tokenStats.reduce((acc, row) => {
    const existing = acc.find(item => row.provider && item.provider === row.provider);
    if (existing) {
	...
	const timeSeriesChartData = timeSeries.reduce((acc, row) => {
	...
	return (
				...
				<Row gutter={16} align="middle">
					...
					<Col flex="auto">
						<RangePicker
							...
							allowClear={false}
						/>
						<Button onClick={() => handleQuickRange(1)} style={{ marginLeft: 8 }}>1 Day</Button>
						<Button onClick={() => handleQuickRange(7)} style={{ marginLeft: 8 }}>7 Days</Button>
						<Button onClick={() => handleQuickRange(30)} style={{ marginLeft: 8 }}>30 Days</Button>
					</Col>
        <Col>
      ...
      {tokenTimeSeries.length > 0 && (
        <Card title="Daily Token Trend" bordered={false} style={{ marginBottom: 16 }}>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={tokenTimeSeries.reduce((acc, row) => {
              const existing = acc.find(item => item.date === row.timestamp);
              if (existing) {
